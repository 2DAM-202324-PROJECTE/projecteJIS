<?php
return[
    "PRODUCTES_TXT" => "Productos",
    "LOGIN_TXT" => "Iniciar sesión",
    "REGISTRE_TXT" => "Registrarse",
    "TRIA_IDIOMA_TXT" => "Idioma",
    "PERFIL_TXT" => "Perfil",
    "TANCAR_SESSIO_TXT" => "Cerrar sesión",
    "ENVIO_GRATIS_HEADER_TXT" => "Envío gratis en pedidos superiores a 250€",
    'CATEGORIES_ARRAY_TXT' => [
        'Torres' => 'Torres',
        'Portàtils' => 'Portátiles',
        'Smartphones' => 'Smartphones',
        'Tablets' => 'Tabletas',
        'Perifèrics' => 'Periféricos',
        'Components' => 'Componentes',
    ],
    "GESTIONAR_COMPTE_TXT" => "Gestionar Cuenta",
    "CATEGORIES_DESCRIPTION_ARRAY_TXT" => [
        'Torres' => 'Ideal para jugar y trabajar en casa.',
        'Portàtils' => 'Ideal para usarlo en cualquier lugar.',
        'Smartphones' => 'Necesario en nuestro día a día.',
        'Tablets' => 'Ideal para ver películas y series.',
        'Perifèrics' => 'Todo lo que necesitas para tu ordenador.',
        'Components' => 'Todo lo que necesitas para montar tu ordenador.',
    ],
    "MISSATGE_PROMOCIONAL_TXT" => "Las mejores ofertas en productos de tecnología!",
    "DESCRIPCIO_MISSATGE_PROMOCIONAL_TXT" => "En nuestra tienda encontrarás las mejores ofertas del mercado! Haz clic en el botón para empezar a comprar!",
    "MOSTRAR_CATEGORIES_TXT" => "Categorías",
    "LES_NOSTRES_CATEG_TXT" => "Nuestras categorías",
    "MISSATGE_PRODUCTES_DESTACATS_TXT" => "Estos son dos de nuestros mejores productos, mira todos los que tenemos disponibles!",

    "BOTO_COMPRA_TOTS_TXT" => "Llévame!",
    "BUSCAR_TXT" => "Buscar...",
    "OFERTES_TXT" => "Ofertas",
    "BOTO_OFERTES_TXT" => "Llévame a ellas",
    "CISTELLA_BUIDA_TXT" => "Tu cesta está vacía",
    "CISTELLA_TXT" => "Tu Cesta",
    "BUIDA_CISTELLA_TXT" => "Vaciar cesta",
    "AFEGIR_CISTELLA_TXT" => "Añadir a cesta",
    "PANEL_ADMIN_TXT" => "Panel de Administración",
    "SUBTOTAL_TXT" => "Subtotal",
    "IVA_TXT" => "IVA",
    "ENVIAMENT_TXT" => "Envío",
    "TOTAL_TXT" => "Total",
    "ACCIONS_TXT" => "Acciones",
    "EDITAR_TXT" => "Editar",
    "ELIMINAR_TXT" => "Eliminar",
    "INICI_TXT" => "Volver a Inicio",
    "NOM_FORM_TXT" => "Nombre",
    "DESCRIPCIO_FORM_TXT" => "Descripción",
    "PREU_FORM_TXT" => "Precio",
    "STOCK_FORM_TXT" => "Stock",
    "IMAGE_URL_FORM_TXT" => "URL de la Imagen",
    "CATEGORY_FORM_TXT" => "Categoría",
    "ESTATE_FORM_TXT" => "Estado",
    "INTROD_IMG_URL_TXT" => "Introduce la URL de la imagen",
    "SELECCIONA_CATEG_TXT" => "Selecciona una categoría",
    "SELECCIONA_ESTAT_TXT" => "Selecciona un estado",
    "CANCELAR_TXT" => "Cancelar",
    "GUARDAR_TXT" => "Guardar",
    "AFEGIR_PRODUCTE_TXT" => "Añadir Producto",
    "CHECKOUT_TXT" => "Finalizar compra",
    "VEURE_PRODUCTES_TXT" => "Ver todos los Productos",
    "TORRES_TXT" => "Torres",
    "DESTACATS_TXT" => "Destacados",
    "SELECT_DESTACATS_TXT" => "Selecciona los Productos destacados",
    "VEURE_CATEGORIES_TXT" => "Ver todas las Categorías",
    "CATEGORIES_TXT" => "Categorías",
    "AFEGIR_CATEGORIA_TXT" => "Añadir Categoría",
    "INTROD_NOM_TXT" => "Introduce el nombre",
    "GRATIS_TXT" => "Gratis",
    "PAGAR_TXT" => "Pagar",
    "RESUM_ORDRE_TXT" => "Resumen del pedido",
    "NOM_TXT" => "Nombre",
    "CORREU_TXT" => "Correo",
    "ADREÇA_TXT" => "Dirección",
    "CIUTAT_TXT" => "Ciudad",
    "ESTAT_TXT" => "Estado",
    "CP_TXT" => "CP",
    "PAIS_TXT" => "País",
    "ESPANYA_TXT" => "España",
    "REGNE_UNIT_TXT" => "Reino Unido",
    "ESTATS_UNITS_TXT" => "Estados Unidos",
    "MARQUES_TXT" => "Marcas",
    "VEURE_MARQUES_TXT" => "Ver todas las marcas",
    "AFEGIR_MARCA_TXT" => "Añadir Marca",
    "COOKIES_DESCRIPCIO_TXT" => "Este sitio web usa cookies para mejorar la experiencia de usuario. Si continúas navegando, consideramos que aceptas su uso.",
    "ACCEPTA_TXT" => "Aceptalo",
    "REBUTJA_TXT" => "Rechazalo",
    "PROMOCIONAL_MARQUES_WELCOME_TXT" => "Tenemos las mejores marcas en productos de tecnología!",
    "DADES_TARGETA_INVALIDES_TXT" => "Por favor, verifica los datos de la tarjeta!",
    "PAGAMENT_COMPLETAT_TXT" => "Pago completado!",
    "AGRAIMENT_COMPRAR_TXT" => "Gracias por comprar en nuestra tienda",
    "TINGUES_BON_DIA_TXT" => "Que tengas un buen día!",
    "TORNAR_BOTIGA_TXT" => "Volver a la tienda",
    "DETALLS_TXT" => "Detalles",
    "MARCA_FORM_TXT" => "Marca",
    "IMAGE_FORM_TXT" => "Imagen",
    "PUJA_FITXER_TXT" => "Subir un archivo",
    "ARRASTRA_SOLTA_TXT" => "o arrastra y suelta",
    "EXPLICACIO_DETALLS_TXT" => "Aquí puedes ver los detalles del producto, para modificarlos, ve al botón de editar.",
    "SELECCIONA_MARCA_TXT" => "Selecciona una marca",
    "ALGO_ERROR_TXT" => "Algo ha ido mal!",
    "REGISTRAT?_TXT" => "Ya estás registrado?",
    "REGISTRAR_TXT" => "Registrar",
    "CONTRASENYA_TXT" => "Contraseña",
    "CONFIRM_CONTRASENYA_TXT" => "Confirmar Contraseña",
    "RECORDA_TXT" => "Recuérdame",
    "OLVIDADA_CONTRA_TXT" => "Has olvidado la contraseña?",
    "NO_COMPTE_TXT" => "No tienes cuenta?",
    "ANTERIOR_TXT" => "Anterior",
    "SEGUENT_TXT" => "Siguiente",
    "FOTO_MARCA_TXT" => "Foto de la Marca",
    "INFO_PERFIL_TXT" => "Información del Perfil",
    "ACTUALITZA_PERFIL_DESCRIPCIO_TXT" => "Actualiza la información de tu perfil y la dirección de correo electrónico.",
    "GUARDAT_TXT" => "Guardado",
    "ACTUALITZAR_CONTRASENYA_TXT" => "Actualizar Contraseña",
    "ASEGURA_PASSWD_SEGURA_TXT" => "Asegúrate de que tu cuenta esté utilizando una contraseña larga y aleatoria para mantenerla segura.",
    "PASSWD_ACTUAL_TXT" => "Contraseña Actual",
    "NOVA_PASSWD_TXT" => "Nueva Contraseña",
    "CONFIRMA_PASSWD_TXT" => "Confirma la Contraseña",
    "ENVIO_DETALLS_TXT" => "Completa los detalles del envío abajo",
    "ENVIO_INFO_TXT" => "Información de Envío",
    "TARGETA_CREDIT_TXT" => "Tarjeta de Crédito",
    "TARGETA_HOLDER_TXT" => "Nombre del titular",
    "NOM_A_LA_TARGETA_TXT" => "Nombre en la tarjeta",
    "EXPIRA_TXT" => "Expira",
    "NUM_TARGETA_TXT" => "Número de tarjeta",
    "DATA_EXP_TXT" => "Fecha de expiración",
    "MES_TXT" => "Mes",
    "ANY_TXT" => "Año",
    "DADES_ENVIO_INCORRECTES_TXT" => "Los datos de envío son incorrectos! Verifícalos e inténtalo de nuevo.",

];
